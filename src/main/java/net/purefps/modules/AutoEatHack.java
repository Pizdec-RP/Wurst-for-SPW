...
public final class AutoEatHack extends Hack implements UpdateListener
{
	...
	public void onUpdate()
	{
		...
		if((isInjured(player) && foodLevel < injuredHungerI) || (foodLevel < minHungerI))
		{
			eat(-1);
			return;
		}
	...
	private int findBestFoodSlot(int maxPoints)
	{
		...
		for(int slot : slots)
		{
			...
			if(!isAllowedFood(food) || (maxPoints >= 0 && food.getHunger() > maxPoints))
				continue;
	...
	{
		if(MC.player.getAbilities().creativeMode || !MC.player.canConsume(false))
			return false;
	...
	private boolean isAllowedFood(FoodComponent food)
	{
		...
		for(Pair<StatusEffectInstance, Float> pair : food.getStatusEffects())
		{
			...
			if((!allowHunger.isChecked() && effect == StatusEffects.HUNGER) || (!allowPoison.isChecked() && effect == StatusEffects.POISON))
				return false;
		}
