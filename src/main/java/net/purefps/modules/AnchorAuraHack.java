...
import net.purefps.settings.FacingSetting.Facing;
import net.purefps.settings.SliderSetting;
import net.purefps.settings.SliderSetting.ValueDisplay;
import net.purefps.settings.filterlists.AnchorAuraFilterList;
...
public final class AnchorAuraHack extends Hack implements UpdateListener
{
	...
	private boolean rightClickBlock(BlockPos pos)
	{
		...
		for(Direction side : Direction.values())
		{
			...
			if((distanceSqHitVec > 36) || (distanceSqHitVec >= distanceSqPosVec) || (checkLOS.isChecked()
	...
	private boolean placeAnchor(BlockPos pos)
	{
		...
		for(Direction side : Direction.values())
		{
			...
			if((eyesPos.squaredDistanceTo(hitVec) > rangeSq) || (distanceSqPosVec > eyesPos.squaredDistanceTo(posVec.add(dirVec))) || (checkLOS.isChecked()
				&& !BlockUtils.hasLineOfSight(eyesPos, hitVec)))
				continue;
