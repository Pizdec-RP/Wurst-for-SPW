...
public final class BonemealAuraHack extends Hack implements UpdateListener
{
	...
	private boolean rightClickBlockLegit(BlockPos pos)
	{
		...
		for(Direction side : Direction.values())
		{
			...
			// check line of sight
			if((distanceSqHitVec > 18.0625) || (distanceSqHitVec >= distanceSqPosVec) || !BlockUtils.hasLineOfSight(eyesPos, hitVec))
				continue;
	...
	private boolean rightClickBlockSimple(BlockPos pos)
	{
		...
		for(Direction side : Direction.values())
		{
			...
			// check if side is facing towards player
			if((distanceSqHitVec > 36) || (distanceSqHitVec >= distanceSqPosVec))
				continue;
