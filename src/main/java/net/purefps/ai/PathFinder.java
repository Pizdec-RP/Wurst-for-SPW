...
{
	private final PFPSClient wurst = PFPSClient.INSTANCE;

	private final boolean invulnerable =
		PFPSClient.MC.player.getAbilities().creativeMode;
	private final boolean creativeFlying =
		PFPSClient.MC.player.getAbilities().flying;
	...
	public PathFinder(BlockPos goal)
	{
		if(PFPSClient.MC.player.isOnGround())
			start = new PathPos(BlockPos.ofFloored(PFPSClient.MC.player.getX(),
				PFPSClient.MC.player.getY() + 0.5,
				PFPSClient.MC.player.getZ()));
		else
			start =
				new PathPos(BlockPos.ofFloored(PFPSClient.MC.player.getPos()));
		this.goal = goal;
	...
	public void think()
	{
		...
		for(; i < thinkSpeed && !checkFailed(); i++)
		{
			...
			current = queue.poll();
			//ChatUtils.message(current.toString());
	...
	private ArrayList<PathPos> getNeighbors(PathPos pos)
	{
		...
		// up
		if(pos.getY() < PFPSClient.MC.world.getTopY() && canGoThrough(up.up())
		...
		// down
		if(pos.getY() > PFPSClient.MC.world.getBottomY() && canGoThrough(down)
	...
	protected boolean isPassable(BlockPos pos)
	{
		...
		BlockPos up = pos.up();
		if((!canGoThrough(up) && !isMineable(up)) || !canGoAbove(pos.down()))
			return false;
	...
	protected boolean isPassableWithoutMining(BlockPos pos)
	{
		...
		BlockPos up = pos.up();
		if(!canGoThrough(up) || !canGoAbove(pos.down()) || (!divingAllowed && BlockUtils.getBlock(up) == Blocks.WATER))
			return false;
	...
	private boolean canGoThrough(BlockPos pos)
	{
		...
		if(!PFPSClient.MC.world.isChunkLoaded(pos))
			return false;
		...
		Block block = state.getBlock();
		// check if trapped
		if((state.blocksMovement() && !(block instanceof AbstractSignBlock)) || block instanceof TripwireBlock
	...
	private boolean canFallBelow(PathPos pos)
	{
		...
		if((immuneToFallDamage && fallingAllowed) || (BlockUtils.getBlock(down2) instanceof SlimeBlock && fallingAllowed))
			return true;
	...
	public boolean isPathStillValid(int index)
	{
		...
		if(invulnerable != PFPSClient.MC.player.getAbilities().creativeMode
