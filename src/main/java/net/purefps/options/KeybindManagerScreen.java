...
public final class KeybindManagerScreen extends Screen
{
	...
	public void init()
	{
		...
		addDrawableChild(ButtonWidget.builder(Text.literal("Reset Keybinds"),
			b -> client.setScreen(new ConfirmScreen(confirmed -> {
				if(confirmed)
					PFPSClient.INSTANCE.getKeybinds()
	...
	private void edit()
	{
		Keybind keybind = PFPSClient.INSTANCE.getKeybinds().getAllKeybinds()
	...
	private void remove()
	{
		Keybind keybind1 = PFPSClient.INSTANCE.getKeybinds().getAllKeybinds()
			.get(listGui.selected);
		PFPSClient.INSTANCE.getKeybinds().remove(keybind1.getKey());
	}
	...
	private static final class ListGui extends ListWidget
	{
		...
		{
			return PFPSClient.INSTANCE.getKeybinds().getAllKeybinds().size();
		}
		...
		{
			Keybind keybind =
				PFPSClient.INSTANCE.getKeybinds().getAllKeybinds().get(index);
