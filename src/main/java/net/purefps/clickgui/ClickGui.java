...
import net.purefps.clickgui.components.FeatureButton;
...
public final class ClickGui
{
	...
	public void init()
	{
		...
		windows.add(PFPSClient.INSTANCE.getHax().radarHack.getWindow());
	...
	public void handleMouseScroll(double mouseX, double mouseY, double delta)
	{
		...
		for(int i = windows.size() - 1; i >= 0; i--)
		{
			...
			if(mouseX < window.getX() || mouseY < window.getY() + 13 || mouseX >= window.getX() + window.getWidth()
		...
		double mouseY, int mouseButton)
	{
		for(int i2 = window.countChildren() - 1; i2 >= 0; i2--)
		{
			...
			if(mouseX < c.getX() || mouseY < c.getY() || mouseX >= c.getX() + c.getWidth()
